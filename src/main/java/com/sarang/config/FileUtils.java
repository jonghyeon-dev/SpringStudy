...
public class FileUtils {
        ...
        try{
            ...
            for(MultipartFile file : uploadFiles){
                ...
                if(saveFile.isFile()){
                    ...
                    //Mimetype Check
                    if(checkFileMimeType(saveFile)){
                        if(saveFile.exists()){
                            if(saveFile.delete()){
                                if(logger.isErrorEnabled()){
                                    logger.error("#FileUpload Interception Check MimeType Diffrent");
                                }
                            }else{
                                if(logger.isErrorEnabled()){
                                    logger.error("#MimeType Check False FileDelete Error FileName : {}.{}", saveFileName,fileExt);
                                }
                            }
                        }
                        return false;
    ...
    public FileVO headContentsFileUpload(HttpSession session,Integer boardId, MultipartFile uploadFile){
        UserVO loginVO = (UserVO) session.getAttribute("userLogin");
        // DB에 저장될 정보
        FileVO fileVO = new FileVO();
        if(ObjectUtils.isEmpty(loginVO)){
    ...
    private static boolean checkFileMimeType(File file) throws Exception{
        boolean isPermision = true;
        try{
            final String[] PERMISSION_FILE_MIME_TYPE = {"image/gif", "image/jpeg", "image/png", "image/bmp", "application/pdf", "video/mp4", "text/plain", "application/zip"};
            ...
            if(logger.isErrorEnabled()){
            logger.error("#MimeType Check Exception Exception Message : {}", e.getMessage());
            }
            return isPermision;
        }
        return isPermision;
