    ...
    <div class="container">
        ...
        <div class="text-center">
            <c:if test="${status eq 'insert'}">
                <c:set var="actionUrl" value="insertHeadContent"/>
            </c:if>
            <c:if test="${status eq 'update'}">
                <c:set var="actionUrl" value="updateHeadContent/${headContentInfo.contentSeq}"/>
            </c:if>
            <form id="boardInsertForm" method="post" action="<c:url value='/admin/${actionUrl}'/>" enctype="multipart/form-data" onSubmit="return validationForm()">
